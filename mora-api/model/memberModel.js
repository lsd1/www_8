...
class MemberModel extends BaseModel{
    ...
    async freezeDiamondDecrement(uid, num, transaction){
        ...
        if(member.freeze_diamond < num){
            let errData = {uid:uid, num:num, memberInfo:member};
            errLog('递减冻结钻石ERR:'+JSON.stringify(errData));
    ...
    //解冻钻石
     async unfreezeDiamond(uid, num, transaction){
        ...
        if(member.freeze_diamond < num){
            let errData = {uid:uid, num:num, memberInfo:member};
            errLog('解冻钻石ERR:'+JSON.stringify(errData));
            throw Error('{"code":110, "msg":"error"}');
        }
         let options = {by: num};
         if(transaction){
