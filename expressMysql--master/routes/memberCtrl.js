...
class memberCtroller{
	static initRouter(){
        ...
        router.get('/delDiamond', async (req, res, next) => {
            try{res.json(await memberService.participateMoraService(req.query.uno, req.query.num));}catch(err){next(err);}
        });

        router.get('/addDiamond', async (req, res, next) => {
            try{res.json(await memberService.cancelMoraService(req.query.uno, req.query.num));}catch(err){next(err);}
        });
