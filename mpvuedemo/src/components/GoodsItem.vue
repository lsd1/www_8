<template>
    <van-swipe-cell @touchstart="onTouchstart"  :id="'swiper' + index" class="goods-swiper" right-width="120" async-close @close="onClose" >
        <div class="goods">
            <div class="mask" v-if="invalid"><span>已失效</span></div>
            <div class="check">
                <checkbox :value="jointKey" :checked="checks.includes(jointKey)" />
            </div>
    ...
    methods: {
      buyNumChange: function (e) {
        ...
        if (e.detail.value <= 0) {
          ...
          this.setGoodsNum(e.detail.value)
        }
      },
      setGoodsNum (num) {
        let _this = this
