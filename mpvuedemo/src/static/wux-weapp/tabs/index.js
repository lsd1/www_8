...
const getDefaultActiveKey = (elements) => {
    ...
    if (target) {
        return target.data.tabKey
    }
    return null
}

const activeKeyIsValid = (elements, tabKey) => {
    return elements.map((element) => element.data.tabKey).includes(tabKey)
}
                ...
                if (condition) {
                    ...
                    elements.forEach((element) => {
                        element.changeCurrent({
                            current: element.data.tabKey === activeKey,
                            scroll,
                            theme,
                            direction,
                        })
