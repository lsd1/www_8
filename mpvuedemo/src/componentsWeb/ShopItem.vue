<template>
    <div class="box">
        <div class="shop">
            <div class="check" >
                <van-checkbox :name="shopKey" @click="checkChange(shopKey)" />
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      async updateNum (data) {
        let _this = this
        console.log('shop num change')
        console.log('this.$parent:', this.$parent.$parent)
        let res = await _this.$parent.$parent.updateNum(data)
        return res
      },
...
<style lang="scss">
    ...
    van-checkbox .wx-van-checkbox-input{
        border-radius: 50%;/* 圆角 */
        width: 40px; /* 背景的宽 */
        height: 40px; /* 背景的高 */
    }
    /* 选中后的 背景样式 （红色背景 无边框 可根据UI需求自己修改） */
    van-checkbox .wx-van-checkbox-input.wx-van-checkbox-input-checked{
        border: none;
        background: red;
    }
    /* 选中后的 对勾样式 （白色对勾 可根据UI需求自己修改） */
    van-checkbox .wx-van-checkbox-input.wx-van-checkbox-input-checked::before{
        border-radius: 50%;/* 圆角 */
