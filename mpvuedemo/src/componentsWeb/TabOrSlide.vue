<template>
    <div>
      <van-tabs class="myTabs" :style="'opacity :'+ opacityCount/10 "  van-class="bordered" :current="current" controlled @change="onTabsChange">
          <template v-for="(item, index) in tabs" >
              <van-tab :key="index" :tabKey="item.key" :title="item.title" />
          </template>
      </van-tabs>
      <slot></slot>
    </div>
