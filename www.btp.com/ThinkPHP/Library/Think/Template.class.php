...
class  Template {
    ...
    public function loadTemplate ($templateFile,$prefix='') {
        ...
        if(strpos($templateFile,MODULE_PATH.'View/core')!==false && file_exists_case(THEME_PATH.'public'.C('TMPL_TEMPLATE_SUFFIX'))){
            $publicContent = file_get_contents(THEME_PATH.'public'.C('TMPL_TEMPLATE_SUFFIX'));
            if(!IS_AJAX){
    ...
    private function parseTemplateName($templateName){
        ...
        if(!is_file($templateName)){
            $templateName = preg_replace('/View\/\w+\//','View/core/',$templateName);//
            if(!is_file($templateName)){
                E('文件解析出错');
            }
        }
