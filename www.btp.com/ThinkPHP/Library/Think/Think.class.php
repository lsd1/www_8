...
class Think {
    ...
    static public function appError($errno, $errstr, $errfile, $errline) {
      switch ($errno) {
          ...
          default:
            ...
            if (defined('WARNING_STATE') && WARNING_STATE){ // APP_DEBUG || IS_CLI || 
              if(defined('MODULE_NAME') && MODULE_NAME == 'Install') return; // NOTIC 会导致安装失败
              if(strpos($errfile,'yubicloud.class.php') !== false && strpos($errstr,'fsockopen') !== false) return; // 过滤yubi url不稳定导致警告
              //dump($errorStr); // 输出NOTIC错误
              break;
            }
      }
    }
    ...
    static public function halt($error) {
        $e = array();
        if (APP_DEBUG || IS_CLI || WARNING_STATE) { // debug_file 为定制化
            //调试模式下输出错误信息
            if (!is_array($error)) {
