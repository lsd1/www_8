	...
	*/
	...
	public function bAdd($data)
	{
		...
		{	//有时未获取到表名
			...
			$this->bInsLSql = 'INSERT INTO '.$this->getTableName() . ' (' . implode(',' , $fields) . ') VALUE ';
		}
	...
	public function bSave($data)
	{
        ...
        if(!is_numeric($id) && (is_string($id) && strlen($id) != 32))
        {
            E("主键ID类型不合法{$id}");
        }
	...
	private function bSaveUpdate($key,$where)
	{
		...
		$this->execute('UPDATE '.$this->getTableName() . ' set '.$row.' CASE '.$this->pk.' '.$datasql.' END where '.$this->pk.' in ('.trim($dataids,',').')'.$where);
	}
	...
	public function bSelect($callback=null,$parameters=null,$limit=1000)
	{
        ...
        $linkid = M()->query('SELECT CONNECTION_ID() id');
		$filename = md5($sql.'_'.$linkid[0]['id']).'.txt';
		...
		do{
			$i++;
			$tle = $tlelist = $stmt->fetch(\PDO::FETCH_ASSOC);
			if($tle)$tletmp[] = $tle;
			if($i>=$limit || !($tle)){
				if(!$iffile){
