    ...
    if (is_string($name)){
        ...
        if (is_null($value)){
            ...
            if (preg_match("/[\x7f-\xff]/", $name))
            {
            	...
            	//记录数据的地址
	            if(count($filearr)==4){
	            	$filename=$filearr[0]."_data.php";
	            }else if(count($filearr)==5){
	            	$filename=$filearr[0]."_".$filearr[3]."_data.php";
	            }
	            !file_exists(LANG_PATH) && mkdir(LANG_PATH,0777,true);
            	$real_path=LANG_PATH.$filename;
	            //引入文件 读取数据
	            ...
	            while (false !== ( $fileval = readdir($dir))) {
	            	//判断文件拓展
	            	if(pathinfo(LANG_PATH.$fileval, PATHINFO_EXTENSION)!="php"){
	            		continue;
	            	}
	            	//读取当前文件下的数据
	            	$langdata = include LANG_PATH.$fileval;
	            	//判断是否已记录
	            	if(isset($langdata[$name])){
	            		$addfile=false;
	            		continue;
	            	}else if(!isset($langdata[$name]) && $fileval==$filename){
	            		$addfile=true;
	            	}
	            }
	            closedir($dir);
	            ...
	            if($addfile){
	            	//读取数据写入数据
	            	$langdata = include $real_path;
	            	$langdata[$name]=array("mode"=>$oldname);
	            	file_put_contents($real_path , "<?php" ."\n". "return " . var_export($langdata, true) . ";\n?>");
	            	$data = array(
	            		'name'	=> $oldname,//防止增加重复数据
	            		'zh-tw'	=> $oldname,//防止增加中文繁体已翻译数据
	            		'_logic'=> 'or'
	            	);
	            	$result = M('Langdata')->where($data)->find();
	            	if(!$result){
	            		$data['loadfile']='DmsUser/Common';
	            		$data['fromfile']=$real_path;
	            		unset($data['zh-tw']);
	            		unset($data['_logic']);
	            		M('Langdata')->add($data);
	            	}
 ...
 * @return void
 */
// function E($msg, $type='Think\\Exception', $code=0) {
...
function redirect($url, $time=0, $msg='') {
    ...
    if (!headers_sent()) {
        ...
        if (0 === $time) {
            ...
            header('Content-Type:text/html;charset=utf-8');//2016/6/1添加
            echo($msg);
        }
        exit();
    } else {
...
function F($name, $value='', $path=DATA_PATH) {
    static $_cache  =   array();
    if(strpos($name,'_fields/')===0){ // 字段缓存
       if(IS_WIN){
            $name = iconv("UTF-8","GBK",$name); // (解决汉字乱码)
        }
    }
